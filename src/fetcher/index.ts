...
import { Chain, BASE_TOKEN, CHAIN_ID, BLOCKS_IN_THE_FUTURE } from '~constants'
...
export const getProfitableOpportunities = async (blockNumber: number) => {
  ...
  return await Promise.all(
    TOKENS_TO_SPOT[CHAIN_ID].map(async (t) => {
      ...
      const result = await math.balance()
      result.log()
      console.log(result.prepareCallData(blockNumber + BLOCKS_IN_THE_FUTURE))
      return result
    })
  )
}
